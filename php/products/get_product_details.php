<?php
// 'get_product_details.php' liefert die Vraiblen für die 'product.php' so das die HTML Elemente gefüllt werden können
// Verbidung zur Datenbank wird durch 'db_connection.php' aufgebaut

require_once('../php/database/db_connection.php');

// Wenn der Nutzer angemeldet ist wird das Profilbild von diesem Nutzer geladen ansonsten wird das default Bild geladen
...
// Die Product_id wird über die URL übergeben
...
for ($i = 1; $i <= 5; $i++) {
    if ($i <= $roundedRating) {
        $stars .= '<i class="material-symbols-outlined md-48 white_stars star_filled rating_star">star</i>';
    } elseif ($i - 0.5 === $roundedRating) {
        $stars .= '<i class="material-symbols-outlined md-48 white_stars star_half rating_star">star_half</i>';
    } else {
        $stars .= '<i class="material-symbols-outlined md-48 white_stars star_empty rating_star">star</i>';
    }
}

// Es werden alle reviews von dem Produkt aus der Datenbank geladen und ausgegeben 
$reviews = '';
...
while ($row = mysqli_fetch_assoc($review_result)) {
    ...
    // Zeigt an ob das Produkt vom Kommentarverfasser gekauft wurde
    $isBought = '';
